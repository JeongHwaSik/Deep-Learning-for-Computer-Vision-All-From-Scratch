...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        flatten_x = x.reshape(num_train, -1) # (N, D)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hidden_out, hidden_cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, last_cache = Linear.forward(hidden_out, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dhidden, dW2, db2 = Linear.backward(dscores, last_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          if i == 0:
            self.params[f'W{i+1}'] = weight_scale * torch.randn(input_dim, hidden_dims[i], dtype=dtype, device=device)
            self.params[f'b{i+1}'] = torch.zeros(hidden_dims[i], dtype=dtype, device=device)
          elif i == self.num_layers - 1:
            self.params[f'W{i+1}'] = weight_scale * torch.randn(hidden_dims[i-1], num_classes, dtype=dtype, device=device)
            self.params[f'b{i+1}'] = torch.zeros(num_classes, dtype=dtype, device=device)
          else:
            self.params[f'W{i+1}'] = weight_scale * torch.randn(hidden_dims[i-1], hidden_dims[i], dtype=dtype, device=device)
            self.params[f'b{i+1}'] = torch.zeros(hidden_dims[i], dtype=dtype, device=device) 
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        cache_data = [] # to store each layer's caches
        for i in range(self.num_layers):
          # first layer
          if i == 0:
            out, cache = Linear_ReLU.forward(X, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
            cache_data.append(cache)
          # last layer; softmax
          elif i == self.num_layers-1:
            scores, last_cache = Linear.forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
            cache_data.append(last_cache)
          else:
        ...
        for i in range(self.num_layers)[::-1]:
          ...
          if i == self.num_layers - 1:
            dhidden, dW, db = Linear.backward(dscores, cache_data[i])
            dW += 2 * self.reg * self.params[f'W{i+1}']
            grads[f'W{i+1}'] = dW
            grads[f'b{i+1}'] = db
          else:
            dhidden, dW, db = Linear_ReLU.backward(dhidden, cache_data[i])
            dW += 2 * self.reg * self.params[f'W{i+1}']
            grads[f'W{i+1}'] = dW
            grads[f'b{i+1}'] = db
        # dx = dhidden
...
def create_solver_instance(data_dict, dtype, device):
    ...
    optim_config={'learning_rate':1e-1, 'reg':1.0, 'hidden_dim' : 500, 'weight_decay' : 1e-2}, device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def rmsprop(w, dw, config=None):
    ...
    # Update parameters using the adaptive learning rate
...
def adam(w, dw, config=None):
    ...
    w -= config['learning_rate'] * moment1_unbias / (torch.sqrt(moment2_unbias) + config['epsilon'])
    next_w = w
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(*x.shape, device='cuda') < (1-p)) / (1-p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
