...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        N, C, H, W = x.shape
        F, C, HH, WW = w.shape
        H_prime = 1 + (H + 2 * pad - HH) // stride # H'
        W_prime = 1 + (W + 2 * pad - WW) // stride # W'

        x_pad = torch.nn.functional.pad(x, (pad,pad,pad,pad), "constant", 0) # zero-padding
        out = torch.zeros((N, F, H_prime, W_prime), dtype=torch.float64, device='cuda') # (N, F, H', W')

        for i in range(N):
          for j in range(F):
            for k in range(H_prime):
              for l in range(W_prime):
                receptive_field = x_pad[i, :, k*stride:k*stride+HH, l*stride:l*stride+WW]
                out[i,j,k,l] = torch.sum(receptive_field * w[j,:,:,:]) + b[j]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
          ...
          for j in range(F):
            for k in range(H_prime):
              for l in range(W_prime):
                ...
                dw[j] += receptive_field * dout[i,j,k,l]
                db[j] += dout[i,j,k,l]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        H_prime = 1 + (H - pool_height) // stride # H'
        W_prime = 1 + (W - pool_height) // stride # W'
        out = torch.zeros(N, C, H_prime, W_prime, dtype=torch.float64, device='cuda')
        
        for i in range(N):
          for j in range(C):
            for k in range(H_prime):
              for l in range(W_prime):
                pooling_region = x[i,j,k*stride:k*stride+pool_height,l*stride:l*stride+pool_width]
                out[i,j,k,l] = torch.max(pooling_region)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
        ...
        for i in range(N):
          for j in range(C):
            for k in range(H_prime):
              for l in range(W_prime):
                pooling_region = x[i,j,k*stride:k*stride+pool_height,l*stride:l*stride+pool_width]
                mask = (pooling_region == torch.max(pooling_region))
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        C, H, W = input_dims
        HH = WW = filter_size
        D = (H//2) * (W//2) * F # flatten dimension after max pooling

        # convolution layer
        self.params['W1'] = weight_scale * torch.randn(F, C, HH, WW, dtype=dtype, device=device) # (F, C, HH, WW)
        self.params['b1'] = torch.zeros(F, dtype=dtype, device=device)# (F,)

        # linear layer1
        self.params['W2'] = weight_scale * torch.randn(D, hidden_dim, dtype=dtype, device=device) # (D, hidden_dim)
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device) # (hidden_dim,)

        # linear layer2
        self.params['W3'] = weight_scale * torch.randn(hidden_dim, num_classes, dtype=dtype, device=device) # (hidden_dim, num_classes)
        self.params['b3'] = torch.zeros(num_classes, dtype=dtype, device=device) # (num_classes,)
    ...
    def loss(self, X, y=None):
        ...
        conv_relu_pool_out, conv_relu_pool_cache = Conv_ReLU_Pool.forward(
          X, self.params['W1'], self.params['b1'], conv_param, pool_param
        )

        # Linear_ReLU layer
        linear_relu_out, linear_relu_cache = Linear_ReLU.forward(
          conv_relu_pool_out, self.params['W2'], self.params['b2']
        )

        # Linear layer
        scores, linear_cache = Linear.forward(
          linear_relu_out, self.params['W3'], self.params['b3']
        )
        ...
        dout, dw1, db1 = Conv_ReLU_Pool.backward(dout, conv_relu_pool_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        # from 1 to (L-1)
        for i, layer in enumerate(range(1, self.num_layers)):
          if layer == 1:
            Din, H, W = input_dims
            Dout = num_filters[i]

            if weight_scale == 'kaiming':
              self.params[f'W{layer}'] = kaiming_initializer(Din, Dout, K=3, relu=True, device=device, dtype=self.dtype)
            else:
              self.params[f'W{layer}'] = weight_scale * torch.randn(Dout, Din, 3, 3, dtype=self.dtype, device=device) # (F, C, 3, 3)

            self.params[f'b{layer}'] = torch.zeros(Dout, dtype=self.dtype, device=device) # (F,)

            # Batch Normalization
            if self.batchnorm:
              self.params[f'gamma{layer}'] = torch.ones((Dout,), dtype=self.dtype, device=device)
              self.params[f'beta{layer}'] = torch.zeros((Dout,), dtype=self.dtype, device=device)
            ...
            if weight_scale == 'kaiming':
              ...
              self.params[f'W{layer}'] = weight_scale * torch.randn(Dout, Din, 3, 3, dtype=self.dtype, device=device)
            
            self.params[f'b{layer}'] = torch.zeros(Dout, dtype=self.dtype, device=device)
          ...
          num_max_pools = len(self.max_pools)
          HH = H // (2**num_max_pools)
          WW = W // (2**num_max_pools)
          if weight_scale == 'kaiming':
            self.params[f'W{self.num_layers}'] = kaiming_initializer(HH*WW*Dout, num_classes, K=None, relu=False, device=device, dtype=self.dtype)
          else:
            self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(HH*WW*Dout, num_classes, dtype=self.dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        # fully connected layer
        scores, last_cache = Linear.forward(out, self.params[f'W{layer+1}'], self.params[f'b{layer+1}'])
        ...
        dout, dw, db = Linear.backward(dscores, last_cache)
        ...
        for i in range(self.num_layers-1)[::-1]:
          if i in self.max_pools:
            if self.batchnorm:
              ...
              dout, dw, db = Conv_ReLU_Pool.backward(dout, cache_list[i])
              dw += 2 * self.reg * self.params[f'W{i+1}']
              grads[f'W{i+1}'] = dw
              grads[f'b{i+1}'] = db
          else:
            if self.batchnorm:
              dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, cache_list[i])
              dw += 2 * self.reg * self.params[f'W{i+1}']
              grads[f'W{i+1}'] = dw
              grads[f'b{i+1}'] = db
              grads[f'gamma{i+1}'] = dgamma
              ...
              dout, dw, db = Conv_ReLU.backward(dout, cache_list[i])
              dw += 2 * self.reg * self.params[f'W{i+1}']
              grads[f'W{i+1}'] = dw
              grads[f'b{i+1}'] = db
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        fan_in = Din
        weight = (gain / fan_in)**(0.5) * torch.randn(Din, Dout, device=device, dtype=dtype)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        # Replace "pass" statement with your code
        fan_in = Din * K * K
        weight = (gain / fan_in)**(0.5) * torch.randn(Dout, Din, K, K, device=device, dtype=dtype)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            # variance (std**2)
            var = torch.var(x, dim=0, correction=0) # (D,)
            running_var = momentum * running_var + (1 - momentum) * var
            bn_param['running_var'] = running_var

            x_norm = (x - mean) / torch.sqrt(var + eps) # (N, D)
    ...
    def backward(dout, cache):
        ...
        # Gradients at Scale and Shift step
        dgamma = torch.sum(dout * x_norm, dim=0)
        dbeta = torch.sum(dout, dim=0)

        #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
        
        # Gradients at the Normalization step
        dx_norm = dout * gamma # (N, D)

        # check out Batch_Norm paper p4
        dvar = torch.sum(dx_norm * (x - mean) * (-0.5) * ((var+eps)**(-1.5)), dim=0) # (D,)
        dmean = torch.sum(dx_norm*(-1.0/torch.sqrt(var+eps)), dim=0) + dvar * torch.sum((-2.0)*(x-mean),dim=0)/N # (D,)
        dx = dx_norm / torch.sqrt(var+eps) + dvar*(2.0*(x - mean) / N) + dmean / N
    ...
    def backward_alt(dout, cache):
        ...
        dgamma = torch.sum(dout * x_norm, dim=0) # (D,)
        dbeta = torch.sum(dout, dim=0) # (D,)

        dy_dmean_dx = (-1.0/torch.sqrt(var+eps)) * (torch.ones_like(x) / N) # (N, D)
        dy_dvar_dx = (-1.0/torch.sqrt(var+eps)) * ((x-mean)**2 / (var+eps))
        dx = torch.sum(dout, dim=0) * dy_dmean_dx + dout * dy_dvar_dx
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        out = out.reshape(N, H, W, C).permute(0,3,1,2) # (N, C, H, W)
    ...
    def backward(dout, cache):
        ...
        N, C, H, W = dout.shape
        dout = dout.permute(0,2,3,1).reshape(-1, C) # (N*H*W, C)
