...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flatten = torch.reshape(x_train, (num_train,-1))
    x_test_flatten = torch.reshape(x_test, (num_test,-1))
    for i in range(num_train):
      for j in range(num_test):
        dis_vector = torch.pow((x_train_flatten[i] - x_test_flatten[j]),2)
        dists[i,j] = torch.sqrt(torch.sum(dis_vector))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten = torch.reshape(x_test, (num_test,-1))
    for i in range(num_train):
      each_x_train_vector = torch.pow((x_test_flatten - x_train_flatten[i]), 2)
      dists[i,:] = torch.sqrt(torch.sum(each_x_train_vector, dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    # This code is not that fast because it increases size of tensor
    # dists = torch.sum((x_train_flatten.unsqueeze(1) - x_test_flatten.unsqueeze(0)).pow(2), dim=2).sqrt()
    #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\

    training_norms = torch.sum(x_train_flatten ** 2, dim=1, keepdim=True)
    test_norms = torch.sum(x_test_flatten ** 2, dim=1, keepdim=True)
    dists = (training_norms - 2 * torch.mm(x_train_flatten, x_test_flatten.t()) + test_norms.t()).sqrt()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      ...
      label = torch.bincount(y_train[indices]).argmax()
      y_pred[i] = label
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for x_chunked, y_chunked in zip(torch.chunk(x_train, num_folds), torch.chunk(y_train, num_folds)):
      x_train_folds.append(x_chunked)
      y_train_folds.append(y_chunked)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
      ...
      for j in range(num_folds):
        ...
        x_validate_data = torch.tensor(x_folds.pop(j))
        y_validate_data = torch.tensor(y_folds.pop(j))
        x_train_data = torch.cat(x_folds[:])
        y_train_data = torch.cat(y_folds[:])
        classifier = KnnClassifier(x_train_data, y_train_data)
        acc = classifier.check_accuracy(x_validate_data, y_validate_data, k)
        acc_list.append(acc)
