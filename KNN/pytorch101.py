...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_idx = torch.tensor([1, 0 ,3])
    y_idx = torch.tensor([0, 1, 2])
    y = x[x_idx, y_idx]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    positive_mask = x>0
    pos_sum = torch.sum(x[positive_mask]).item() # rank-1 tensor 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for batch in range(batch_size):
      results.append(x[batch].mm(y[batch]))
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M, N = x.shape
    y = x.clone() # (M, N)
    mean = y.sum(dim=0) / M # (N,)
    std = (torch.sum((y - mean)**2, dim=0)/(M-1)).sqrt() # (N,)
    y = (y - mean) / std # (M, N)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    cum_idx = torch.cumsum(ls, dim=0) - 1
    sum_of_x = cum_sum_x[cum_idx]
    sum_x_alpha = sum_of_x[:-1]
    sum_of_x[1:] = sum_of_x[1:] - sum_x_alpha
    y = sum_of_x / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    sorted_idx = torch.zeros_like(x)
    torch.sort(x, out=(x, sorted_idx)) # return sorted values & indices
    
    t = torch.cat((torch.tensor([True]), x[1:] != x[:-1]), 0)
    uniques = x[t]
    indices = sorted_idx[t]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
