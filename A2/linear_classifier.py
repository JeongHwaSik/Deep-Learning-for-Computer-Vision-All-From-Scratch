...
def svm_loss_naive(
    ...
    for i in range(num_train): # N
        ...
        for j in range(num_classes): # C
            ...
            if margin > 0:
                ...
                #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
                # Analytic Gradient of Loss function

                # Accumulate gradient for incorrect classes
                dW[:, j] += X[i]
                # Accumulate gradient for correct class
                dW[:, y[i]] -= X[i]
                #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    ...
    # Average gradients over the training examples
    dW /= num_train
...
def svm_loss_vectorized(
    ...
    correct_scores = scores[torch.arange(X.shape[0]),y].unsqueeze(1)
    margins = torch.maximum(torch.zeros_like(scores), scores - correct_scores + 1)
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    random_idx = torch.randint(0, num_train-1, (batch_size,))
    X_batch = torch.tensor(X[random_idx])
    y_batch = torch.tensor(y[random_idx])
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    predicted = torch.mm(X, W)
    _, y_pred = torch.max(predicted, dim=1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e-2, 1e0, 1e1, 1e2, 1e3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      ...
      correct_class_probs = probs[y[i]]

      loss -= torch.log(correct_class_probs)

      #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\

      # Analytic Gradient of Softmax
...
def softmax_loss_vectorized(
    ...
    loss = torch.sum(-probs[torch.arange(num_examples), y]) / num_examples
    loss += reg * torch.sum(W * W)
