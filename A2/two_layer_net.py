...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    scores = torch.mm(hidden, W2) + b2 # (N, C)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    grads['W2'] = torch.transpose(h1, 0, 1).mm(dscores) + 2 * reg * W2 # (H, C)
    grads['b2'] = dscores.sum(dim=0)
    dhidden = dscores.mm(torch.transpose(W2, 0, 1)) # (N, H)

    dhidden[h1 == 0] = 0

    grads['W1'] = torch.transpose(X, 0, 1).mm(dhidden) + 2 * reg * W1 # (D, H)
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        # by using sample_batch, can't we iterate all the examples??
      ...
      for n_hidden in hidden_sizes:
        for reg in regularization_strengths:
          for lr_decay in learning_rate_decays:
            net = TwoLayerNet(3*32*32,n_hidden, 10, dtype=data_dict['X_train'].dtype, device=data_dict['X_train'].device)
            ...
            if best_val_acc < max(stats['val_acc_history']):
              best_net = net
              best_stat = stats
