...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for stem_channel in stem_channels:
          conv = nn.Conv2d(in_channels, stem_channel, kernel_size=3, stride=1, padding=1)
        ...
        # Replace "pass" statement with your code
        self.pred_obj = nn.Conv2d(stem_channels[-1], 1*num_anchors, kernel_size=3, stride=1, padding=1)
        self.pred_box = nn.Conv2d(stem_channels[-1], 4*num_anchors, kernel_size=3, stride=1, padding=1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level, feats in feats_per_fpn_level.items():
          obj = self.pred_obj(self.stem_rpn(feats)) # (B, k=num_anchors, H, W)
          batch_size, k, height, width = obj.shape

          object_logits[level] = obj.permute(0,2,3,1).contiguous().view(batch_size, -1) # (B, H * W * k)

          boxreg = self.pred_box(self.stem_rpn(feats)) # (B, 4k, H, W)

          boxreg = boxreg.view(batch_size, 4, k, height, width) # (B, 4, k, H, W)
          boxreg = boxreg.contiguous().permute(0, 1, 3, 4, 2).contiguous().view(batch_size, 4, -1) # (B, 4, H * W * k)
          # (B, H * W * k, 4)
          boxreg_deltas[level] = boxreg.permute(0,2,1)
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            # locations_per_fpn_level: {'p3': (H*W, 2), 'p4': ~, 'p5': ~}
            x = locations_per_fpn_level[level_name][:,0]
            y = locations_per_fpn_level[level_name][:,1]
            ...
            # (A, H * W, 4)
            anchor_boxes.append(torch.stack((top,left,bottom,right), dim=-1)) # (x1, y1, x2, y2)
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    for i in range(M):
      for j in range(N):
        box1_left, box1_top, box1_right, box1_bottom = boxes1[i, :]
        box2_left, box2_top, box2_right, box2_bottom = boxes2[j, :]

        # each area
        box1 = (box1_right - box1_left) * (box1_bottom - box1_top)
        box2 = (box2_right - box2_left) * (box2_bottom - box2_top)
        
        # intersection
        width = max(min(box1_right, box2_right) - max(box1_left, box2_left), 0)
        height = max(min(box1_bottom, box2_bottom) - max(box1_top, box2_top), 0)
        intersection = width * height

        total = box1 + box2 - intersection

        iou[i, j] = intersection / total
...
def rcnn_match_anchors_to_gt(
    ...
    # TODO: compare my IoU implementation(BottleNeck here!!!) with torchvision.ops.box_iou
    match_matrix = torchvision.ops.box_iou(anchor_boxes, gt_boxes[:, :4]) # (N, M)

    # Find matched ground-truth instance per anchor:
    match_quality, matched_idxs = match_matrix.max(dim=1) # (N,)
...
def rcnn_apply_deltas_to_anchors(
    ...
    output_boxes = torch.stack((left, top, right, bottom), dim=-1)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        # FPN location coordinates
        shape_per_fpn_level = {level: feats.shape for level, feats in feats_per_fpn_level.items()}
        locations_per_fpn_level = get_fpn_location_coords(shape_per_fpn_level, strides_per_fpn_level, device=device)

        # generate FPN anchors ------------------------------------------------------------------------------------------
        ...
        # Dict -> single tensor (N, 4), N: # of all anchors
        ...
        for batch_idx in range(gt_boxes.shape[0]):
          matched_gt_box = rcnn_match_anchors_to_gt(anchor_boxes, gt_boxes[batch_idx], self.anchor_iou_thresholds)
        ...
        matched_gt_boxes = torch.stack(matched_gt_boxes, dim=0) # (B, 3087, 5)

        # Combine predictions across all FPN levels.
        pred_obj_logits = self._cat_across_fpn_levels(pred_obj_logits) # (B, 3087,)
        pred_boxreg_deltas = self._cat_across_fpn_levels(pred_boxreg_deltas) # (B, 3087,4)

        if self.training:
            # Repeat anchor boxes `batch_size` times so there is a 1:1
            # correspondence with GT boxes.
            # (3087, 4) -> (3087, 4, 1) -> (B*3087, 4, 1) -> (B*3087, 4)
            ...
            matched_gt_boxes = matched_gt_boxes.view(-1, 5) # (B*3087, 5)
            pred_obj_logits = pred_obj_logits.view(-1) # (B*3087,)
            pred_boxreg_deltas = pred_boxreg_deltas.view(-1, 4) # (B*3087, 4)
            ...
            # ignore neutral, only background & foreground
            fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, num_samples=2 * self.batch_size_per_image, fg_fraction=0.5)
            ...
            loss_obj = F.binary_cross_entropy_with_logits(pred_obj_logits[sample_idx], matched_gt_boxes[sample_idx, 4], reduction="none")
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            # Get anchor boxes(XYXY) and predictions from a single level.
            level_anchors = anchors_per_fpn_level[level_name] # (HWA, 4: x1,y1,x2,y2)
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                # anchors
                px = (level_anchors[:, 0] + level_anchors[:, 2]) / 2
                py = (level_anchors[:, 1] + level_anchors[:, 3]) / 2
                pw = level_anchors[:, 2] - level_anchors[:, 0]
                ph = level_anchors[:, 3] - level_anchors[:, 1]

                # deltas (transform)
                dx = level_boxreg_deltas[_batch_idx, :, 0] # tx
                dy = level_boxreg_deltas[_batch_idx, :, 1] # ty
                dw = level_boxreg_deltas[_batch_idx, :, 2] # tw
                dh = level_boxreg_deltas[_batch_idx, :, 3] # th

                # proposals
                proposal_x = px + pw * dx
                proposal_y = py + ph * dy
                proposal_w = pw * torch.exp(dw)
                proposal_h = ph * torch.exp(dh)

                # proposal boxes
                proposal_x1 = proposal_x - (proposal_w / 2) # left
                proposal_y1 = proposal_y - (proposal_h / 2) # top
                proposal_x2 = proposal_x + (proposal_w / 2) # right
                proposal_y2 = proposal_y + (proposal_h / 2) # bottom

                # clamp to image height and width
                width, height = image_size
                proposal_x1 = proposal_x1.clamp(min=0, max=width)
                proposal_y1 = proposal_y1.clamp(min=0, max=height)
                proposal_x2 = proposal_x2.clamp(min=0, max=width)
                proposal_y2 = proposal_y2.clamp(min=0, max=height)

                # (HWA, 4) -> (XYXY)
                proposal_boxes = torch.stack((proposal_x1,proposal_y1,proposal_x2,proposal_y2), dim=-1)

                # 2. Choose top k='self.pre_nms_topk' proposal boxes
                if len(level_obj_logits[_batch_idx, :]) < self.pre_nms_topk:
                  topk_idx = torch.topk(level_obj_logits[_batch_idx, :], k=len(level_obj_logits[_batch_idx, :])).indices
                else:
                  topk_idx = torch.topk(level_obj_logits[_batch_idx, :], k=self.pre_nms_topk).indices

                topk_proposals = proposal_boxes[topk_idx, :] # (k, 4)
                topk_scores = level_obj_logits[_batch_idx, topk_idx] # (k,)

                # 3. Apply NMS and retain `keep_topk_per_level` proposals per image, per level.
                keep_topk_per_level = torchvision.ops.nms(topk_proposals, topk_scores, self.nms_thresh)
                
                proposals_per_image = topk_proposals[keep_topk_per_level] # (~, 4)
                
                level_proposals_per_image.append(proposals_per_image)
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        # Replace "pass" statement with your code
        for stem_channel in stem_channels:
            conv = nn.Conv2d(stem_channel, stem_channel, kernel_size=3, stride=1, padding=1)
            nn.init.normal_(conv.weight, mean=0, std=0.01)
            nn.init.zeros_(conv.bias)
        ...
        linear = nn.Linear(stem_channels[-1] * roi_size[0] * roi_size[1], num_classes+1)
    ...
    def forward(
        ...
        for _idx in range(len(gt_boxes)):
            ...
            # Replace "pass" statement with your code
            matched_gt_boxes.append(rcnn_match_anchors_to_gt(anchor_boxes=proposals_per_image, gt_boxes=gt_boxes_per_image, iou_thresholds=(0.5,0.5)))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, num_samples=self.batch_size_per_image, fg_fraction=0.25)
        sample_idx = torch.cat((fg_idx, bg_idx))

        # if you change with .type(torch.LongTensor), it will move tensor to the CPU
        sampled_matched_gt_boxes = (matched_gt_boxes[sample_idx, 4].type(torch.LongTensor) + 1).to(sample_idx.device)
        loss_cls = F.cross_entropy(pred_cls_logits[sample_idx], sampled_matched_gt_boxes)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return {
    ...
    def inference(
        ...
        # Step 1:
        pred_scores = nn.Softmax(dim=-1)(pred_cls_logits)

        pred_classes = pred_scores.max(dim=1).indices
        pred_classes -= 1
        pred_scores = pred_scores.max(dim=1).values

        # Step 2:
        keep = (pred_scores > test_score_thresh)  # (keep,)
