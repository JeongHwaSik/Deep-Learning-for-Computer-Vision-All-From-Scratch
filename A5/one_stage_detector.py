...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
          p = feats_per_fpn_level[key]

          # (B, H * W, num_classes)
          class_out = self.pred_cls(self.stem_cls(p)) # (B, num_classes, H, W)
          B, num_classes, H, W = class_out.shape
          class_logits[key] = class_out.reshape(B, num_classes, H*W).permute(0, 2, 1)
          # (B, H * W, 4)
          boxreg_out = self.pred_box(self.stem_box(p)) # (B, 4, H, W)
          boxreg_deltas[key] = boxreg_out.reshape(B, 4, H*W).permute(0, 2, 1)
          # (B, H * W, 1)
          ctr_out = self.pred_ctr(self.stem_box(p)) # (B, 1, H, W)
          centerness_logits[key] = ctr_out.reshape(B, 1, H*W).permute(0, 2, 1)
...
def fcos_get_deltas_from_locations(
    ...
    is_background = (x1 == -1) & (y1 == -1) & (x2 == -1) & (y2 == -1)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    nominator = torch.min(left, right) * torch.min(top, bottom)
    denominator = torch.max(left, right) * torch.max(top, bottom)
    centerness = torch.sqrt(nominator / denominator)

    is_background = (left == -1) & (top == -1) & (right == -1) & (bottom == -1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # returns {}'p3', 'p4', 'p5'}
        # 1. Classification logits: `(B, H * W, num_classes)`.
        # 2. Box regression deltas: `(B, H * W, 4)`
        # 3. Centerness logits:     `(B, H * W, 1)`
        ...
        matched_gt_boxes = [] # will be [{'p3': (784,5), 'p4': (196,5),'p5': (49,5)}, {'p3': ~, 'p4': ~,'p5': ~}, ...], len()=batch_size
        # Replace "pass" statement with your code
        
        batch_size = gt_boxes.shape[0] # B

        for batch in range(batch_size):
          matched_gt_box = fcos_match_locations_to_gt(locations_per_fpn_level, strides_per_fpn_level, gt_boxes[batch,:,:])
          matched_gt_boxes.append(matched_gt_box)

        # Calculate GT deltas for these matched boxes. Similar structure
        # as `matched_gt_boxes` above. Fill this list:
        matched_gt_deltas = [] # will be [{'p3': (784,4), 'p4': (196,4),'p5': (49,4)}, {'p3': ~, 'p4': ~,'p5': ~}, ...], len()=batch_size
        # Replace "pass" statement with your code

        for matched_gt_box in matched_gt_boxes: # B
          deltas = {}
          for level, stride in strides_per_fpn_level.items():
            delta = fcos_get_deltas_from_locations(locations_per_fpn_level[level], matched_gt_box[level], stride)
            deltas[level] = delta
        ...
        # shape: (batch_size, num_locations_across_fpn_levels(784+196+49), ...)
        matched_gt_boxes = self._cat_across_fpn_levels(matched_gt_boxes) # (B, N, 5)
        matched_gt_deltas = self._cat_across_fpn_levels(matched_gt_deltas) # (B, N, 4)
        pred_cls_logits = self._cat_across_fpn_levels(pred_cls_logits) # (B, N, num_classes)
        pred_boxreg_deltas = self._cat_across_fpn_levels(pred_boxreg_deltas) # (B, N, 4)
        pred_ctr_logits = self._cat_across_fpn_levels(pred_ctr_logits) # (B, N, 1)
        ...
        # 1. centerness loss (binary cross entropy loss)
        gt_ctr_logits = []
        for batch in range(batch_size):
          gt_ctr = fcos_make_centerness_targets(matched_gt_deltas[batch,:,:]) # (1029,)
          gt_ctr_logits.append(gt_ctr)
        gt_ctr_logits = torch.stack(gt_ctr_logits).unsqueeze(dim=-1) # (B, 1029, 1)
        # (B, 1029, 1)
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, gt_ctr_logits, reduction='none')
        # background positions : loss to zero
        loss_ctr[gt_ctr_logits < 0] *= 0.0

        # 2. box regression loss (L1 loss)
        loss_box = F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction='none') # (B, 1029, 4)
        # background positions : loss to zero
        loss_box[matched_gt_deltas < 0] *= 0.0
        
        # 3. classification loss (focal loss)
        # predict : (B, 1029, num_classes)
        # GT : (B, 1029,) -> GT_onehot : (B, 1029, num_classes)
        batch_size, num_locations, num_classes = pred_cls_logits.shape
        gt_cls_onehot = torch.zeros(batch_size, num_locations, num_classes, device='cuda') # (B, 1029, num_classes)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_locations = locations_per_fpn_level[level_name] # (H * W, 2)
            ...
            # Step 1:
            # Replace "pass" statement with your code
            level_pred_classes = level_pred_scores.max(dim=1).indices # (N, )
            level_pred_scores = level_pred_scores.max(dim=1).values # (N, )

            # Step 2:
            # Replace "pass" statement with your code
            keep = (level_pred_scores > test_score_thresh) # (keep,)
            
            keep = keep.cuda()
            level_locations = level_locations.cuda()

            level_pred_scores = level_pred_scores[keep] # (keep, )
            level_pred_classes = level_pred_classes[keep] # (keep, )
            level_locations = level_locations[keep] # (keep, 2)
            level_deltas = level_deltas[keep] # (keep, 4)

            # Step 3:
            # Replace "pass" statement with your code
            stride = self.backbone.fpn_strides[level_name]
            # (keep, 4) -> (LTRB)
            pred_boxes = fcos_apply_deltas_to_locations(level_deltas, level_locations, stride)
            ...
            level_pred_boxes = torch.stack((x1, y1, x2, y2), dim=-1) # (keep, 4)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
