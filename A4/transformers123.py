...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    out = torch.zeros_like(query) # (K, M)

    for i in range(K):
      similarity_vector = []
      for j in range(K):
        dot_product = torch.dot(query[i], key[j]) / (M**0.5) # scalar
        similarity_vector.append(dot_product)
      similarity_vector = torch.tensor(similarity_vector) # row vector: (K,)
      attn_weights = F.softmax(similarity_vector)
      out[i] = torch.matmul(attn_weights, value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.zeros_like(query) # (N, K, M)

    for n in range(N):
      for i in range(K):
        sim = torch.matmul(query[n][i], key[n].T) / (M**0.5) # (K,)
        attn_weights = F.softmax(sim) # (K,)
        out[n][i] = torch.matmul(attn_weights, value[n])
...
def scaled_dot_product_no_loop_batch(
    ...
    # the weghts that don't need to be masked at that position. These masked-scaled#
    # weights can then be softmaxed to compute the final weighted sum of values   #
    # Hint: look at torch.bmm and torch.masked_fill                               #
    ###############################################################################
    # Replace "pass" statement with your code

    sim = torch.bmm(query, key.transpose(-2, -1)) / (M**0.5) # similarities: (N, K ,K)
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(sim, dim=-1) # attn weights: (N, K, K)
    y = torch.bmm(weights_softmax, value) # output: (N, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.q = nn.Linear(dim_in, dim_q) # query
        nn.init.xavier_uniform_(self.q.weight) 
        self.k = nn.Linear(dim_in, dim_q) # key
        nn.init.xavier_uniform_(self.k.weight)
        self.v = nn.Linear(dim_in, dim_v) # value
    ...
    def forward(
        ...
        # get the final output. For now, don't worry about the mask and just      #
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        transformed_query = self.q(query) # (N, K, dim_q)
        transformed_key = self.k(key) # (N, K, dim_q)
        transformed_value = self.v(value) # (N, K, dim_v)
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(transformed_query, transformed_key, transformed_value, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # 1.) Use nn.ModuleList to initialize a list of SingleHeadAttention layer #
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        """
        query: (N, K, dim_in) -> (N, K, dim_out = dim_q)
        key: (N, K, dim_in) -> (N, K, dim_out = dim_q)
        value: (N, K, dim_in) -> (N, K, dim_out = dim_v)
        similarities: (N, K, K)
        output: (N, K, dim_out) -> num_heads SingleHeadAttention, concat -> (N, K, num_heads*dim_out)
        y: (N, K, dim_in)
        """
        out = []
        for head in self.multihead:
          single_output = head(query, key, value, mask) # (N, K, dim_out)
          out.append(single_output)
        concat_y = torch.cat(out, dim=-1) # (N, K, num_heads * dim_out)
        y = self.linear(concat_y) # (N, K, dim_in)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        # with appropriate dimensions. Don't forget to encapsulate these scale and#
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        mean = torch.mean(x, dim=-1, keepdim=True) # mean along the last dimension: (N, K, 1) or (N, 1)
        var = torch.mean((x - mean)**2, dim=-1, keepdim=True) # std = sqrt(var): (N, K, 1) or (N, 1)
        z = (x - mean) / (torch.sqrt(var) + self.epsilon) # (N, K, M) or (N, K)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.MLP_layer1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        nn.init.xavier_uniform_(self.MLP_layer1.weight)
        self.relu = nn.ReLU()
        self.MLP_layer2 = nn.Linear(hidden_dim_feedforward, inp_dim)
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        """
        (N, K, inp_dim) -> MLP1 -> (N, K, hidden_dim_feedforward) -> ReLU
        -> (N, K, hidden_dim_feedforward) -> MLP2 -> (N, K, input_dim)
        """
        y = self.MLP_layer2(self.relu(self.MLP_layer1(x)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        """
        See 'Attention Is All You need' papaer Figure 1:

        input -> multi_head_attention -> output1 -> layer_norm(output1 + input) 
        -> dropout -> output2 -> feedforward -> output3 
        -> layer_norm(output3 + output2) -> dropout -> out
        """

        output1 = self.multihead(x, x, x)
        output2 = self.dropout(self.layernorm(output1 + x))
        output3 = self.feedforward(output2)
        y = self.dropout(self.layernorm(output2 + output3))
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.triu(torch.ones((K, K), device=seq.device).to(bool), diagonal=1).expand(N, -1, -1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # Replace "pass" statement with your code
        # MultiHeadAttention(num_heads, dim_in, dim_out)
    ...
    def forward(
        ...
        layer_norm(out5 + out4) - dropout - out
        """
        output1 = self.attention_self(dec_inp, dec_inp, dec_inp, mask)
        output2 = self.dropout(self.norm1(output1 + dec_inp))
        output3 = self.attention_cross(output2, enc_inp, enc_inp)
        output4 = self.dropout(self.norm2(output3 + output2))
        output5 = self.feed_forward(output4)
        y = self.dropout(self.norm3(output5 + output4))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.zeros(K, 1) # (K, 1)
    for k in range(K):
      y[k, :] = k / K
    y = y.repeat(1, M).unsqueeze(0) # (K, M) -> (1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for p in range(K):
      for i in range(M):
        if i % 2 == 0: # i = EVEN
          a = i // M
          PE = np.sin(p / pow(10000, a))
          y[p][i] = PE
        else:         # i = ODD
          a = (i-1) // M
          PE = np.cos(p / pow(10000, a))
          y[p][i] = PE
    y = y.unsqueeze(0) # (K, M) -> (1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    def __init__(
        ...
        the output sequence length(K_out - 1) and V is the Vocabulary size. The input 
        is passed through shared nn.Embedding layer and then added to input positonal 
        encodings. Similarily, the target is passed through the same nn.Embedding
        layer and added to the target positional encodings. 
        
        /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
        The only difference is that we take last but one value in the target.
        /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
        
        The summed inputs(look at the code for details) are then sent through the encoder 
        and decoder blocks to get the final output.
    ...
    def forward(
        ...
        """
        q_emb = self.emb_layer(ques_b) # (N, K_in, M)
        a_emb = self.emb_layer(ans_b) # (N, K_out, M)
        q_emb_inp = q_emb + ques_pos # (N, K_in, M)
        a_emb_inp = a_emb[:, :-1] + ans_pos[:, :-1] # (N, K_out - 1, M)
        dec_out = None
        ##########################################################################
        ...
        dec_out = self.decoder(a_emb_inp, enc_out, mask) # (N, K_out - 1, vocab_len)
        dec_out = dec_out.reshape(-1, dec_out.shape[-1]) # reshape to (N * (K_out - 1), V)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class AddSubDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inp = self.input_seqs[idx] # Ex) "BOS POSITIVE 20 add NEGATIVE 44 EOS"
        out = self.target_seqs[idx] # Ex) "BOS NEGATIVE 24 EOS"
        preprocess_inp = torch.tensor(self.preprocess(inp)) # Ex) tensor([14, 10, 2, 0, 12, 11, 4, 4, 15])
        preprocess_out = torch.tensor(self.preprocess(out)) # Ex) tensor([[14, 11, 2, 4, 15]])
        inp_pos = len(preprocess_inp) # K_in = 9
        inp_pos_enc = self.pos_encode(inp_pos, self.emb_dim) # (1, K_in, M)
        out_pos = len(preprocess_out) # K_out = 5
        out_pos_enc = self.pos_encode(out_pos, self.emb_dim) # (1, K_out, M)

        return preprocess_inp, inp_pos_enc[0], preprocess_out, out_pos_enc[0]

    def __len__(self):
        return len(self.input_seqs) # N : total number of samples
