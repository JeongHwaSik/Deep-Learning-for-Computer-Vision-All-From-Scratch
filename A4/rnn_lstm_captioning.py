...
def rnn_step_backward(dnext_h, cache):
    ...
    draw_h = (1 - next_h**2) * dnext_h # bofore tanh function: (N, H)
    db = torch.sum(draw_h, dim=0) # (H,) : copy gate!
    dWh = torch.mm(prev_h.T, draw_h) # (H, H)
    dprev_h = torch.mm(draw_h, Wh.T) # (N, H)
    dWx = torch.mm(x.T, draw_h) # (D, H)
    dx = torch.mm(draw_h, Wx.T) # (N, D)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0 # (N, H)
    for t in range(T):
      x_step = x[:,t,:] # (N, D) : a word inside of a sequence
      next_h, cache_t = rnn_step_forward(x_step, prev_h, Wx, Wh, b)
      h[:,t,:] = next_h
      prev_h = next_h
      cache[t] = cache_t
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    N, T, H = dh.shape
    init_h, next_h, x0, Wx0, Wh0 = cache[0]
    D = x0.shape[1]

    dx = torch.zeros((N, T, D), dtype=x0.dtype, device=x0.device)
    dh0 = torch.zeros_like(init_h)
    dWx = torch.zeros_like(Wx0)
    dWh = torch.zeros_like(Wh0)
    db = torch.zeros((H,), dtype=x0.dtype, device=x0.device)

    dnext_h_t = torch.zeros_like(init_h)
    for t in reversed(range(T)):
      # copy gate!!! : Upstream gradients for output states + Upstream gradients for hidden states
      dprev_h_t = dh[:,t,:] + dnext_h_t # (N, H)
      dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dprev_h_t, cache[t])
      dnext_h_t = dprev_h
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    Args:
        ...
        y: Ground-truth indices, of shape (N, T) where each element is in the
            range 0 <= y[i, t] < V
        ignore_index: integer of <NULL> index
    ...
    loss = 0
    for t in range(T):
      loss += F.cross_entropy(x[:,t,:], y[:,t], ignore_index=ignore_index, reduction='sum') / N
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.image_encoding_layer = ImageEncoder()
    ...
    def forward(self, images, captions):
        ...
        # image: (N, C_in, HH, WW) -> features: (N, C_out, H/32, W/32)
        features = self.image_encoding_layer.forward(images) 
        if self.cell_type == 'rnn' or self.cell_type == 'lstm':
          # average pooling: (N, C_out)=(2,400)
          avg_pooled_features = torch.mean(features.view(features.size(0), features.size(1), -1), dim=2)
          # affine transformation of feature vector
          h0 = self.feature_layer(avg_pooled_features) # (N, H)
        elif self.cell_type == 'attn':
          A = self.feature_layer(features.permute(0,2,3,1)).permute(0,3,1,2)
          
        # word embedding layer
        x = self.word_embedding_layer.forward(captions_in) # (N, T, W)

        # RNN / LSTM layer
        if self.cell_type == 'rnn':
          # RNN
          hs = self.rnn.forward(x, h0) # hidden states: (N, T, H)
        elif self.cell_type == 'lstm':
          # LSTM
          hs = self.lstm.forward(x, h0)
        elif self.cell_type == 'attn':
          # LSTM with attention
          hs = self.attn.forward(x, A)
        # affine transformation for outputs
        scores = self.output_layer(hs) # output scores: (N, T, V)
    ...
    def sample(self, images, max_length=15):
        ...
        # image: (N, C_in, H, W) -> features: (N, C_out, H/32, W/32) -> average pooling: (N, C_out)
        features = self.image_encoding_layer.forward(images)
        if self.cell_type == 'rnn' or self.cell_type == 'lstm':
          avg_pooled_features = torch.mean(features.view(features.size(0), features.size(1), -1), dim=2)
          # initialize h0
          prev_h = self.feature_layer(avg_pooled_features) # h0: (N, H)
          # if LSTM
          if self.cell_type == 'lstm':
            prev_c = torch.zeros_like(prev_h) # (N, H)
        elif self.cell_type == 'attn':
          A = self.feature_layer(features.permute(0,2,3,1)).permute(0,3,1,2)
        ...
        # initialize x: (N, V)
        prev_word = torch.zeros((N, len(self.word_to_idx)), device=prev_h.device)
        prev_word[:, self._start] = 1

        for t in range(max_length):
          # word embedding layer
          prev_word = torch.mm(prev_word, self.word_embedding_layer.W_embed) # (N, W)

          if self.cell_type == 'rnn':
            # RNN
            next_h = self.rnn.step_forward(prev_word, prev_h) # (N, H)
            prev_h = next_h
          elif self.cell_type == 'lstm':
            # LSTM
            next_h, next_c = self.lstm.step_forward(prev_word, prev_h, prev_c)
            prev_h = next_h
            prev_c = next_c
          elif self.cell_type == 'attn':
            # LSTM with attention
            ...
            next_h, next_c = self.attn.step_forward(prev_word, prev_h, prev_c, attn)
            prev_h = next_h
            prev_c = next_c

          # affine transformation for outputs
          scores = self.output_layer(next_h) # (N, V)
          next_word = torch.zeros_like(scores)
          next_word[torch.arange(N), torch.argmax(scores, dim=1)] = 1
          prev_word = next_word
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        a = torch.mm(x, self.Wx) + torch.mm(prev_h, self.Wh) + self.b # (N, 4H)
        ai, af, ao, ag = torch.split(a, H, dim=1)
        input_gate = F.sigmoid(ai) # (N, H)
        forget_gate = F.sigmoid(af) # (N, H)
        output_gate = F.sigmoid(ao) # (N, H)
        gate_gate = F.tanh(ag) # (N, H)

        next_c = forget_gate * prev_c + input_gate * gate_gate # (N, H)
        next_h = output_gate * F.tanh(next_c) # (N, H)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        Note that the initial hidden state is passed as input, but the initial
        ...
        prev_h = h0 # initialize h0: (N, H)
        prev_c = c0 # initialize c0: (N, H)
        for t in range(T):
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    flatten_A = A.reshape((A.shape[0], A.shape[1], -1)) # (N, H, 4, 4) -> (N, H, 16)
    # attention scores
    attn_scores = torch.matmul(prev_h.unsqueeze(1), flatten_A) / (H**0.5) # (N, 1, 16)
    attn_scores = attn_scores.reshape(N, -1) # (N, 16)
    # attnetion weights
    attn_weights = F.softmax(attn_scores, dim=1).reshape(N, 4, 4) # (N, 4, 4)
    # attention embedding output
    attn = torch.sum(A * attn_weights.unsqueeze(1), dim=(2,3)) # (N, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        a = torch.mm(x, self.Wx) + torch.mm(prev_h, self.Wh) + torch.mm(attn, self.Wattn) + self.b # (N, 4H)
        ai, af, ao, ag = torch.split(a, H, dim=1)
        input_gate = F.sigmoid(ai) # (N, H)
        forget_gate = F.sigmoid(af) # (N, H)
        output_gate = F.sigmoid(ao) # (N, H)
