...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device="cpu"):
    ...
    noise = (-2) * torch.rand(batch_size, noise_dim, device='cuda') + 1
...
def discriminator_loss(logits_real, logits_fake):
    ...
    real_true_labels = torch.ones((batch_size, 1), device='cuda')
    fake_true_labels = torch.zeros((batch_size, 1), device='cuda')

    real_logits_loss = \
    nn.functional.binary_cross_entropy_with_logits(logits_real, real_true_labels, reduction='sum') / batch_size

    fake_logits_loss = \
    nn.functional.binary_cross_entropy_with_logits(logits_fake, fake_true_labels, reduction='sum') / batch_size

    loss = real_logits_loss + fake_logits_loss
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def generator_loss(logits_fake):
    ...
    true_labels = torch.ones((batch_size, 1), device='cuda')

    loss = \
    nn.functional.binary_cross_entropy_with_logits(logits_fake, true_labels, reduction='sum') / batch_size
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def get_optimizer(model):
    ...
    # Replace "pass" statement with your code
    optimizer = \
    torch.optim.Adam(model.parameters(), lr=1e-3, betas=(0.5, 0.999))
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return optimizer
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    batch_size = scores_real.shape[0]

    loss = torch.sum((scores_real-1).pow(2) + scores_fake.pow(2)) / (2*batch_size)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def ls_generator_loss(scores_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    batch_size = scores_fake.shape[0]

    loss = torch.sum((scores_fake-1).pow(2)) / (2*batch_size)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
