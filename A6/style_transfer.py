...
def content_loss(content_weight, content_current, content_original):
    ...
    # current image
    F = content_current.view(C_l, H_l * W_l) # (C_l, M_l)

    # original image
    P = content_original.view(C_l, H_l * W_l) # (C_l, M_l)
...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.bmm(features, features.transpose(dim0=1, dim1=2)) # (N, C, C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer in enumerate(style_layers):
      A_l = gram_matrix(feats[layer], normalize=True)
      G_l = style_targets[idx]
      
      style_loss += style_weights[idx] * torch.sum((G_l - A_l)**2)
...
def tv_loss(img, tv_weight):
    ...
    loss = tv_weight * (torch.sum(vertical_diff**2) + torch.sum(horizontal_diff**2))
